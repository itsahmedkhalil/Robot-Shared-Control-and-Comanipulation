...
class DynamicPlotter():
    def __init__(self, sampleinterval=0.1, timewindow=10., size=(600,350)):
        ...
        self.posXplot = self.p10.plot(self.x, self.posX, pen=(255,255,0))
        self.posYplot = self.p11.plot(self.x, self.posY, pen=(255,255,0))
        self.posZplot = self.p12.plot(self.x, self.posZ, pen=(255,255,0))
        # QTimer
    ...
    def getdata(self):
        ...
        if decoded_bytes[0] != 'C':
            ...
            #rows = np.array(acc).reshape(3,1)
            #f_hat = np.linalg.inv(M) * (np.array(rows) - b_a)
            #f_hat = f_hat.reshape(3)
            #acc = np.asarray(f_hat)
